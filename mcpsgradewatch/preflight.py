...
Run it without anything else configured::
    ...
    mcpsgradewatch preflight --district <portal-host>            # anonymous
    mcpsgradewatch preflight --district <portal-host> -u USER    # full check
"""
...
VERDICT_TEXT = {
    "go": "Gate PASSED — the portal speaks the protocol and the parsers "
          "understand its fragments. MCPSGradeWatch should work here.",
...
def render_markdown(report: Report) -> str:
    ...
    lines = [
        ...
        f"| Tool | mcpsgradewatch preflight v{report.version} |",
    ...
    lines += ["", f"> {VERDICT_TEXT[report.verdict]}",
              "",
              "_Generated by `mcpsgradewatch preflight --report`. "
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    district = args.district or os.environ.get("MCPSGRADEWATCH_DISTRICT")
    if not district:
        print("error: no district — pass --district HOST or set MCPSGRADEWATCH_DISTRICT",
              file=sys.stderr)
        return 2
    username = None if args.anonymous else (
        args.username or os.environ.get("MCPSGRADEWATCH_USERNAME"))
    ...
    if username is None:
        ...
        backend = os.environ.get("MCPSGRADEWATCH_SECRET_BACKEND", "keyring")
        try:
