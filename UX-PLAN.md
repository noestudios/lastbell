...
### Grade trends over time — BACKLOGGED (owner's call 2026-09-01)
Deliberately parked until after launch; the leftovers below are the
backlog. The data model and first sparklines shipped with C0.
...
### School-name links — DONE (2026-09-01)
Every rendered instance of a school name (overview cards, student page
header) links to that school's own website. Resolved at render time via a
**bundled static map** (`lastbell/mcps_schools.json`, regenerated by
`scripts/refresh_schools.py`); `schools.py` normalizes both sides (level
words like HS/High/High School fold to one token) so Synergy's name forms
match the directory's, and a miss degrades silently to plain text.

### Cross-cutting — human-readable errors — DONE (2026-09-01)
Inventoried every user-facing failure and rewrote each as plain language +
what happens next:
- **CLI top level**: LoginError / ParseError / ParentVueError /
  RequestException now exit as one plain line with the fix (check
  credentials · run preflight and file a district report · check the
  network) — a traceback is a bug report, not an error message.
  Ctrl-C exits clean ("stopped.", code 130).
- **Login failure** message itself carries the fix (re-store password,
  try a browser sign-in — MFA/CAPTCHA blocks automated login), so both
  the one-shot and loop paths teach it.
- **Loop poll failure**: "couldn't reach the portal (…) — retrying at
  HH:MM" (the plan's own example wording, with the real retry time).
- **Collect warnings** say the blast radius: class skipped this poll,
  others still checked.
- **Delivery warnings** (router/outbox/summary) say whether the message
  retries, and no-address warnings teach the `set-channel` fix.
- **Dashboard**: 500 explains it's usually momentary + refresh; 404s
  explain and link back. Settings `?err=` banners were already human
  (watchers.py messages teach their own fixes).
Already good and untouched: ConfigError/SecretError texts,
`validate_address` (carrier-gateway teaching), preflight verdicts.
